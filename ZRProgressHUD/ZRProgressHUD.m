...
static ZRProgressHUD *_progressHUD;


@interface ZRProgressHUD ()
...
+ (ZRProgressHUD *)shareInstance
{
    dispatch_once(&onceToken, ^{
        _progressHUD = [[ZRProgressHUD alloc]init];
    });
    return _progressHUD;
}
...
- (void)showMessage:(NSString *)message withCompletion:(ZRProgressHUDCompletion)completion
{
    self.message = message;
...
- (void)showMessage:(NSString *)message withErrorImage:(UIImage *)errorImage
{
    if (errorImage) {
        ...
        //ZRLog(@"您选定的图片为空，已自动设成默认图片");
        self.tipImage = [UIImage imageNamed:@"ZR_error"];
    }
    
    [self showMessage:message];
}

- (void)showMessage:(NSString *)message withErrorImage:(UIImage *)errorImage withCompletion:(ZRProgressHUDCompletion)completion
{
    if (errorImage) {
        self.tipImage = errorImage;
    }else {
...
- (void)showMessage:(NSString *)message withSuccessImage:(UIImage *)successImage
{
    if (successImage) {
        ...
        //ZRLog(@"您选定的图片为空，已自动设成默认图片");
        self.tipImage = [UIImage imageNamed:@"ZR_success"];
    }
...
- (void)showMessage:(NSString *)message withSuccessImage:(UIImage *)successImage withCompletion:(ZRProgressHUDCompletion)completion
{
    if (successImage) {
        self.tipImage = successImage;
    }else {
...
- (void)dismissWithCompletion:(ZRProgressHUDCompletion)completion
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.animationValue * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.5 animations:^{
            [self.tipImageView removeFromSuperview];
...
- (void)initSelf
{
    //ZRLog(@"原始的：%f",self.tipImageViewWH);
